...
const Dashboard = () => {
  ...
  const handleUpdateAgency = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`/api/agencies/${user.agencyId}`, {
  ...
  return (
    <div style={{ padding: '2rem 4rem', maxWidth: '1400px', margin: '0 auto' }}>
      <header className="fade-in" style={{ marginBottom: '3rem' }}>
        <h1 style={{ fontSize: '3.5rem', fontWeight: 800, marginBottom: '0.5rem', letterSpacing: '-2px' }}>Agency <span style={{ color: 'var(--primary)' }}>Dashboard</span></h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: '2.5rem' }}>
        ...
        <section className="glass fade-in" style={{ gridColumn: 'span 2', padding: '2.5rem', marginBottom: '4rem' }}>
          ...
          {incidents.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '2rem' }}>
              {incidents.map(incident => (
                <div key={incident._id} className="glass" style={{ padding: '2rem', background: 'rgba(255, 255, 255, 0.02)', border: '1px solid rgba(255, 255, 255, 0.05)' }}>
